...
Internal ports must be `1024..65535`. External port `0` disables the separate Gate endpoint (the default for new profiles); `1024..65535` enables it and must differ from Internal port. Reserved values `1..1023` stay closed. Unified continues using the internal endpoint independently. Use `--no-apply` to save without restarting a running MCP.
...
`unified-port=0` disables Unified and remains disabled across restarts. Values `1..1023` also remain closed (invalid/red in the UI). This does not disable separate Gate ports or stop MCP processes.
