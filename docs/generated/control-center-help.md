...
- **External port** is optional: `0` (new-profile default) disables the separate Gate listener, `1024..65535` enables it. Apply profile port changes with Start/Restart. Existing configured ports are not reset during updates.
- **Unified port** is independent: `0` disables Unified after pressing **Gate**, without stopping MCPs or their separate Gate endpoints. Unified connects directly to internal endpoints.
- A disabled separate endpoint shows **Gate: Disabled**; its Copy Gate URL action is unavailable. Status and Health continue to describe the MCP process/internal endpoint. `0` is neutral; reserved ports `1..1023` are red and never opened.
